    ...
    <div className="mx-auto max-w-5xl px-4 py-12">
      <div className="mb-10">
        ...
        <p className="mt-4 max-w-2xl text-gray-600 dark:text-gray-400">
          Explore tactical analysis and football articles organised by player.
        </p>
      </div>

      {players.length === 0 ? (
        <p className="text-gray-600 dark:text-gray-400">No players have been added yet.</p>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {players.map((player) => (
            <Link
              key={player}
              href={`/players/${slug(player)}`}
