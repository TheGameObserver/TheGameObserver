...
export default function TacticalTopicsPage() {
  const tacticalTopics = Array.from(
    new Set(
      allCoreContent(sortPosts(allBlogs))
        .flatMap((post) => post.tacticalTopics || [])
    ...
    <div className="mx-auto max-w-5xl px-4 py-12">
      <div className="mb-10">
        ...
        <p className="mt-4 max-w-2xl text-gray-600 dark:text-gray-400">
          Explore tactical analysis and football articles organised by tactical concept.
        </p>
      </div>

      {tacticalTopics.length === 0 ? (
        <p className="text-gray-600 dark:text-gray-400">No tactical topics have been added yet.</p>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {tacticalTopics.map((topic) => (
            <Link
              key={topic}
              href={`/tactical-topics/${slug(topic)}`}
